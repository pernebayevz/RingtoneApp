...
struct NetworkManager {
    ...
    func getMain(completion: @escaping (_ MainModel: MainModel?,_ error: String?)->()){
        router.request(.main) { data, response, error in
            if error != nil {
                completion(nil, "Please check your network connection.")
            }
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case .success:
                    ...
                    do {
                        ...
                        let jsonData = try JSONSerialization.jsonObject(with: responseData, options: .mutableContainers)
                        print(jsonData)
                        let apiResponse = try JSONDecoder().decode(MainModel.self, from: responseData)
